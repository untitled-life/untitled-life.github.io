---
layout: post
title: "An Introduction to the Presto"
date: 2019-02-25 12:19:25 +0800
comments: true
categories: Presto
---
...
## 连接器(Connector)
Presto是通过各种连接器(Connector)来访问不同的数据源的，可以将连接器(Connector)当作Presto访问各种数据源的驱动程序。连接器(Connector)是Presto的SPI的实现，允许Presto使用标准API与资源进行交互。
Presto包含多个内置连接器(Connector)，并且允许第三方开发自定义连接器(Connector)，以便Presto能够访问各种数据源中的数据。
## 目录(Catalog)
